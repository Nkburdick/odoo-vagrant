...
echo "Welcome to odoo-8 installer for ubuntu 14.04"
...
git clone --depth=1 --branch=$OE_VERSION https://github.com/odoo/odoo.git ~/src/odoo > /dev/null

print_out "Install pip dependencies for odoo (it may take a log time)"
sudo -H pip install --upgrade pip > /dev/null
sudo -H pip install -r ~/src/odoo/requirements.txt > /dev/null
