...
#odoo
OE_USER="vagrant"
OE_HOME="/$OE_USER/odoo-dev"
OE_HOME_EXT="/$OE_USER/odoo-dev/odoo"
...
sudo sed -i s/"db_user = .*"/"db_user = $OE_USER"/g $OE_HOME/config/${OE_CONFIG}.conf
sudo sed -i s/"; admin_passwd.*"/"admin_passwd = $OE_SUPERADMIN"/g $OE_HOME/config/${OE_CONFIG}.conf
sudo su root -c "echo 'logfile = /var/log/$OE_USER/$OE_CONFIG$1.log' >> $OE_HOME/config/${OE_CONFIG}.conf"
sudo su root -c "echo 'addons_path=$OE_HOME_EXT/addons,$OE_HOME/custom/addons' >> $OE_HOME/config/${OE_CONFIG}.conf"

echo -e "* Create startup file"
sudo su root -c "echo '#!/bin/sh' >> $OE_HOME_EXT/start.sh"
sudo su root -c "echo 'sudo -u $OE_USER $OE_HOME_EXT/openerp-server --config=$OE_HOME/config/${OE_CONFIG}.conf' >> $OE_HOME_EXT/start.sh"
...
CONFIGFILE="$OE_HOME/config/${OE_CONFIG}.conf"
...
sudo su root -c "echo 'xmlrpc_port = $OE_PORT' >> $OE_HOME/config/${OE_CONFIG}.conf"
